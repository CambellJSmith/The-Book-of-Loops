...
import { blank_card, example_card, element_colors, type MonsterCard } from "@/lib/card_model"; // shares collection record types.
  ...
  return ( // composes the collection panel from sidebar primitives.
    <Sidebar collapsible="none" className="collection_panel" aria-label="card collection">
      ...
      <SidebarContent className="collection_content">
        ...
        <div className="collection_caption">collection</div>
        {loading ? <div className="collection_loading"><Skeleton className="h-20 w-full" /><span>loading your cards…</span></div> : error ? <div className="collection_error" role="alert"><p>{error}</p><Button variant="outline" size="sm" onClick={() => void reload()}>try_again</Button></div> : <SidebarMenu>{cards.map((card: MonsterCard) => <SidebarMenuItem key={card.id}><SidebarMenuButton className="saved_card_item" isActive={current.id === card.id} disabled={disabled} onClick={() => select(card)} aria-label={`open ${card.species_name}, species ${card.species_id}`}><span className="collection_art" style={{ borderColor: element_colors[card.type] }}>{card.art ? <img src={card.art} alt="" loading="lazy" /> : <ImageIcon size={19} />}</span><span className="collection_card_text"><strong>{card.species_name}</strong><span>#{card.species_id} · {card.difficulty}</span><small style={{ color: element_colors[card.type] }}><ElementIcon element={card.type} size={12} />{card.type}</small></span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu>}
      ...
      <SidebarFooter className="collection_footer"><span className="element_spectrum">{(["fire", "water", "nature", "light", "dark"] as const).map((element) => <ElementIcon key={element} element={element} size={15} style={{ color: element_colors[element] }} />)}</span><span>five types. your world.</span></SidebarFooter>
    </Sidebar>
  );
}
