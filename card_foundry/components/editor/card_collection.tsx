...
import { blank_card, difficulties, difficulty_labels, example_card, element_colors, type Difficulty, type MonsterCard } from "@/lib/card_model"; // shares collection records and progression labels.
...
export function CardCollection({ cards, current, loading, error, disabled, select, reload, close }: CollectionProps) { // presents saved cards without creating sample records.
  const grouped_cards = (difficulty: Difficulty): MonsterCard[] => cards.filter((card: MonsterCard) => card.difficulty === difficulty).sort((left: MonsterCard, right: MonsterCard) => left.species_id.localeCompare(right.species_id)); // keeps every difficulty section in stable species order.
  const card_item = (card: MonsterCard) => <SidebarMenuItem key={card.id}><SidebarMenuButton className="saved_card_item" isActive={current.id === card.id} disabled={disabled} onClick={() => select(card)} aria-label={`open ${card.species_name}, species ${card.species_id}, ${difficulty_labels[card.difficulty]}${card.is_final_boss ? ", final boss" : ""}`}><span className="collection_art" style={{ borderColor: element_colors[card.type] }}>{card.art ? <img src={card.art} alt="" loading="lazy" /> : <ImageIcon size={19} />}</span><span className="collection_card_text"><strong>{card.species_name}</strong><span>#{card.species_id}{card.is_final_boss ? " · final boss" : ""}</span><small style={{ color: element_colors[card.type] }}><ElementIcon element={card.type} size={12} />{card.type} / {difficulty_labels[card.difficulty]}</small></span></SidebarMenuButton></SidebarMenuItem>; // renders one saved monster with its element and encounter band.
  return ( // composes the collection panel from sidebar primitives.
    <Sidebar collapsible="none" className="collection_panel" aria-label="card collection">
      ...
      <SidebarContent className="collection_content">
        ...
        <div className="collection_caption">difficulty_groups</div>
        {loading ? <div className="collection_loading"><Skeleton className="h-20 w-full" /><span>loading your cards…</span></div> : error ? <div className="collection_error" role="alert"><p>{error}</p><Button variant="outline" size="sm" onClick={() => void reload()}>try_again</Button></div> : difficulties.map((difficulty: Difficulty) => { const group: MonsterCard[] = grouped_cards(difficulty); return group.length ? <div key={difficulty}><div className="collection_caption">{difficulty_labels[difficulty]} · {group.length}</div><SidebarMenu>{group.map(card_item)}</SidebarMenu></div> : null; })}
        ...
        {!loading && <div className="example_area"><div className="collection_caption">starting_point</div><button className="example_card_button" disabled={disabled} onClick={() => select({ ...example_card, id: crypto.randomUUID(), species_id: blank_card(cards).species_id })}><img src="/sample_art.webp" alt="cindrel example artwork" /><span><strong>cindrel</strong><small>editable example · easy</small></span><ElementIcon element="fire" size={15} /></button></div>}
      </SidebarContent>
      <SidebarFooter className="collection_footer"><span className="element_spectrum">{(["fire", "water", "nature", "light", "dark"] as const).map((element) => <ElementIcon key={element} element={element} size={15} style={{ color: element_colors[element] }} />)}</span><span>five types. five difficulties.</span></SidebarFooter>
    </Sidebar>
  );
}
