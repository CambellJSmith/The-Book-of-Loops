import { card_schema, type Element, type MonsterCard, type Move } from "@/lib/card_model"; // shares validated player-facing monster fields with the editor.
...
export interface CardObject { species_id: string; species_name: string; type: Element; is_final_boss: boolean; art: ObjectArtwork | null; art_prompt: string; health: number; speed: number; moves: [Move, Move]; } // exports player-facing card data without internal encounter difficulty or editor-only database state.
export interface CardObjectDocument { format: "card_foundry"; schema_version: 3; cards: CardObject[]; assets: Record<string, EmbeddedArtwork>; } // versions the portable file format after removing internal progression metadata from user exports.
...
export async function create_card_objects(records: readonly MonsterCard[], progress?: ExportProgress, load_artwork: ArtworkLoader = embed_artwork): Promise<CardObjectDocument> { // converts complete cards into an independent, versioned object document.
  ...
  const objects: CardObject[] = cards.map((card: MonsterCard) => ({ // preserves user-facing game data independently from the current editor implementation.
    species_id: card.species_id, species_name: card.species_name, type: card.type, is_final_boss: card.is_final_boss, // keeps identifiers, element and encounter role explicit without exposing internal difficulty.
  ...
  return { format: "card_foundry", schema_version: 3, cards: objects, assets }; // returns a JSON-compatible object containing player-facing card fields and referenced artwork only.
}
