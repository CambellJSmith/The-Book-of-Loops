...
export const difficulties = ["easy", "medium", "hard", "extra_hard", "ultra"] as const; // defines the five requested encounter bands in ascending order.
export type Difficulty = (typeof difficulties)[number]; // restricts difficulty values to the supported progression bands.
export const difficulty_labels: Record<Difficulty, string> = { easy: "easy", medium: "medium", hard: "hard", extra_hard: "extra hard", ultra: "ultra" }; // exposes readable labels without storing spaces in data keys.
...
export const card_schema = z.object({ // defines the complete monster record.
  ...
  difficulty: z.enum(difficulties).default("easy"), // records the encounter band while keeping historical payloads readable.
...
export const example_card: MonsterCard = { // supplies an editable example without inserting records.
  id: "00000000-0000-4000-8000-000000000001", species_id: "0001", species_name: "cindrel", type: "fire", difficulty: "easy", is_final_boss: false, // establishes the sample species and its introductory encounter band.
...
export function blank_card(cards: MonsterCard[]): MonsterCard { // makes a fresh unsaved monster.
  return { ...example_card, id: crypto.randomUUID(), species_id: next_species_id(cards), species_name: "", difficulty: "easy", is_final_boss: false, art: "", art_prompt: "", health: 100, speed: 50, moves: [{ name: "", mana_cost: 1, damage: 10 }, { name: "", mana_cost: 2, damage: 30 }] }; // separates new card data from the sample with an explicit starting difficulty.
}
