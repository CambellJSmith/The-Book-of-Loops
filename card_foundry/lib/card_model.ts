...
export const difficulties = ["easy", "medium", "hard", "extra_hard", "ultra"] as const; // defines the five internal encounter bands in ascending order.
export type Difficulty = (typeof difficulties)[number]; // restricts internal difficulty values to the supported progression bands.
export const difficulty_labels: Record<Difficulty, string> = { easy: "easy", medium: "medium", hard: "hard", extra_hard: "extra hard", ultra: "ultra" }; // keeps internal labels available to progression logic without making them player-facing.
...
export function move_two_is_stronger(moves: readonly { mana_cost: number; damage: number }[]): boolean { // enforces the canonical cheap-first, strong-second attack ordering.
  return moves.length === 2 && moves[1].mana_cost > moves[0].mana_cost && moves[1].damage > moves[0].damage; // requires move two to cost more mana and deal more damage than move one.
}
export const card_schema = z.object({ // defines the complete monster record.
  ...
  difficulty: z.enum(difficulties).default("easy"), // records the internal encounter band while keeping historical payloads readable.
  is_final_boss: z.boolean().default(false), // identifies the one canonical climactic encounter independently from difficulty.
...
}).superRefine((card, context) => { // validates relationships between the two move slots after their individual fields are checked.
  if (card.moves[1].mana_cost <= card.moves[0].mana_cost) context.addIssue({ code: z.ZodIssueCode.custom, path: ["moves", 1, "mana_cost"], message: "move 2 must cost more mana than move 1" }); // preserves the turn-based mana progression.
  if (card.moves[1].damage <= card.moves[0].damage) context.addIssue({ code: z.ZodIssueCode.custom, path: ["moves", 1, "damage"], message: "move 2 must deal more damage than move 1" }); // keeps the more expensive second move strictly stronger.
});
...
export const example_card: MonsterCard = { // supplies an editable example without inserting records.
  id: "00000000-0000-4000-8000-000000000001", species_id: "0001", species_name: "cindrel", type: "fire", difficulty: "easy", is_final_boss: false, // establishes the sample species and its introductory internal encounter band.
  ...
  moves: [{ name: "ember flick", mana_cost: 1, damage: 20 }, { name: "flare spiral", mana_cost: 3, damage: 60 }], // demonstrates the cheaper first move and stronger second move.
};
...
export function blank_card(cards: MonsterCard[]): MonsterCard { // makes a fresh unsaved monster.
  return { ...example_card, id: crypto.randomUUID(), species_id: next_species_id(cards), species_name: "", difficulty: "easy", is_final_boss: false, art: "", art_prompt: "", health: 100, speed: 50, moves: [{ name: "", mana_cost: 1, damage: 10 }, { name: "", mana_cost: 2, damage: 30 }] }; // creates a valid cheap-first, strong-second move pair for new records.
}