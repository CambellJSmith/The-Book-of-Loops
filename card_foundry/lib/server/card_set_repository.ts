...
import { apply_card_balance } from "./card_balance_repository"; // upgrades previously installed collection statistics exactly once.
...
export async function install_standalone_cards(): Promise<void> { // installs the requested set atomically while preserving subsequent edits and deletions.
  ...
  if (installed) { await apply_card_balance(); return; } // keeps the user-owned collection authoritative after installation.
  ...
  await apply_card_balance(); // records the same upgrade marker for fresh and existing collections.
}
