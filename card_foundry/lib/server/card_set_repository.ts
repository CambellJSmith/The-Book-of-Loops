...
export async function install_standalone_cards(): Promise<void> { // installs or rebalances the canonical set without restoring user-deleted cards.
  ...
  const patches = cards.map((card: MonsterCard) => ({ id: card.id, difficulty: card.difficulty, is_final_boss: card.is_final_boss, health: card.health, speed: card.speed, moves: card.moves, ...(card.is_final_boss ? { art_prompt: card.art_prompt } : {}) })); // limits migration changes to progression fields while preserving names, artwork and framing.
  ...
  await db.batch([ // applies the canonical rebalance and its completion marker atomically.
    db.prepare("UPDATE cards SET payload = json_patch(payload, (SELECT patch.value FROM json_each(?) AS patch WHERE json_extract(patch.value, '$.id') = cards.id)), revision = revision + 1 WHERE id IN (SELECT json_extract(value, '$.id') FROM json_each(?))").bind(patch_json, patch_json), // updates only surviving canonical card identities so user deletions and uploaded art remain untouched.
