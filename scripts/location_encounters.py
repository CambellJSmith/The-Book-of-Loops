...
def placement_cost(monster: dict, location: dict, progress: float, biases: dict[str, int]) -> float:
    if location["id"] in START_IDS and monster["difficulty"] != "easy":
        return float("inf")
    allowed = allowed_difficulties(progress)
    difficulty_penalty = 0.0 if monster["difficulty"] in allowed else 5_000.0
    progression_cost = abs(progress - TARGET_PROGRESS[monster["difficulty"]]) * 260.0
    strongest = max(biases.values())
    type_gap = strongest - biases[monster["type"]]
    type_penalty = type_gap * 34.0
    tie_break = (stable_hash(f"{monster['species_id']}:{location['id']}") % 1000) / 1000.0
    return difficulty_penalty + progression_cost + type_penalty + tie_break


def choose_monster_for_location(
    available: list[dict], location: dict, progress: float, biases: dict[str, int]
) -> dict:
    allowed = allowed_difficulties(progress)
    candidates = [monster for monster in available if monster["difficulty"] in allowed]
    if location["id"] in START_IDS:
        candidates = [monster for monster in candidates if monster["difficulty"] == "easy"]
    if not candidates:
        candidates = [monster for monster in available if not (location["id"] in START_IDS and monster["difficulty"] != "easy")]
    return min(candidates, key=lambda monster: placement_cost(monster, location, progress, biases))
...
def choose_location_for_monster(
    ...
    return min(
        candidates,
        key=lambda location: placement_cost(monster, location, progress[location["id"]], biases[location["id"]]),
    )
...
def optimize_assignments(
    ...
    for _ in range(30):
        pass_swaps = 0
        for left_index, left_location in enumerate(locations):
            left_id = left_location["id"]
            for right_location in locations[left_index + 1:]:
                right_id = right_location["id"]
                best_swap: tuple[float, int, int] | None = None
                for left_monster_index, left_monster in enumerate(assignments[left_id]):
                    left_to_right = placement_cost(left_monster, right_location, progress[right_id], biases[right_id])
                    if not (left_to_right < float("inf")):
                        continue
                    for right_monster_index, right_monster in enumerate(assignments[right_id]):
                        right_to_left = placement_cost(right_monster, left_location, progress[left_id], biases[left_id])
                        if not (right_to_left < float("inf")):
                            continue
                        old_cost = (
                            placement_cost(left_monster, left_location, progress[left_id], biases[left_id])
                            + placement_cost(right_monster, right_location, progress[right_id], biases[right_id])
                        )
                        new_cost = left_to_right + right_to_left
                        delta = new_cost - old_cost
                        if delta < -0.01 and (best_swap is None or delta < best_swap[0]):
                            best_swap = (delta, left_monster_index, right_monster_index)
                if best_swap is None:
                    continue
                _, left_monster_index, right_monster_index = best_swap
                assignments[left_id][left_monster_index], assignments[right_id][right_monster_index] = (
                    assignments[right_id][right_monster_index], assignments[left_id][left_monster_index]
                )
                pass_swaps += 1
                swap_count += 1
        if pass_swaps == 0:
...
def distribute_encounters(map_data: dict, monsters: list[dict]) -> dict:
    ...
    if sum(capacities.values()) != 399:
        raise RuntimeError(f"ordinary location capacities must total 399, got {sum(capacities.values())}")
    ...
    for location in ordinary_locations:
        for _ in range(3):
            chosen = choose_monster_for_location(available, location, progress[location["id"]], biases[location["id"]])
    ...
    swap_count = optimize_assignments(ordinary_locations, progress, biases, assignments)
    ...
    themed_locations = 0
    themed_locations_with_majority = 0
    for location in ordinary_locations:
        ...
        if highest >= 7 and highest - second_highest >= 4:
            themed_locations += 1
            if preferred_count >= (len(values) + 1) // 2:
...
def patch_index(html: str, map_data: dict) -> str:
    ...
    normalize_block = html.split("function normalizeLocation", 1)[1].split("function migrateParsedData", 1)[0]
    if "encounter_progress:" not in normalize_block:
        html = html.replace(
...
def write_manifest(map_data: dict, distribution: dict, monsters: list[dict]) -> None:
    ...
    manifest = {
        ...
        "summary": {
            ...
            "strongly_themed_location_majority_rate": distribution["themed_location_majority_rate"],
            "optimization_swaps": distribution["swap_count"],
...
def main() -> None:
    ...
    print(f"strongly themed locations with majority-preferred encounters: {distribution['themed_location_majority_rate']:.1%}")
    print(f"global ecological optimization swaps: {distribution['swap_count']}")
